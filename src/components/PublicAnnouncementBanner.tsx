...
export function HomeAnnouncementBanner({ 
  ...
  const [dismissedIds, setDismissedIds] = useState<Set<string>>(
    () => new Set(JSON.parse(localStorage.getItem('dismissedHomeAnnouncements') || '[]'))
  );

  const { data: announcements = [], isLoading, error } = useQuery({
    queryKey: ['announcements', 'home'],
    queryFn: fetchHomeAnnouncements,
  ...
  const handleDismiss = (id: string) => {
    ...
    localStorage.setItem('dismissedHomeAnnouncements', JSON.stringify([...newDismissedIds]));
  };
...
// Hook for getting home announcements data
